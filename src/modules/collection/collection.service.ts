...
export default class CollectionService {
    ...
    async addDocToCollection(collectionId: string, data: CreateDocumentRequestDTO) {
        try {
            ...
            if ((!data.document && !data.url) || !data.type) {
                throw new BadRequestException('Document or URL along with type is required');
            }
            ...
            if ((data.document || data.url) && data.type) {
                try {
                    ...
                    throw new BadRequestException(`Failed to parse document: ${parserError.message}`);
                }
            ...
            this.handleError(error, `Failed to add document to collection "${collectionId}"`);
        }
    }
    ...
    async deleteDocsByMetadata(collectionId: string, data: DeleteDocsRequestDTO) {
        try {
            ...
            this.handleError(error, `Failed to delete documents by metadata from collection "${collectionId}"`);
        }
    }
    ...
    async getDocById(collectionId: string, docId: string) {
        try {
            ...
            if (!doc) {
                throw new NotFoundException(`Document with ID "${docId}" not found in collection "${collectionId}"`);
            }

            return doc;
        } catch (error) {
            this.handleError(error, `Failed to retrieve document "${docId}" from collection "${collectionId}"`);
        }
    }
    ...
    async updateDoc(collectionId: string, docId: string, data: UpdateDocumentRequestDTO) {
        try {
            ...
            this.handleError(error, `Failed to update document "${docId}" in collection "${collectionId}"`);
        }
    }
    ...
    async deleteDoc(collectionId: string, docId: string) {
        try {
            ...
            this.handleError(error, `Failed to delete document "${docId}" from collection "${collectionId}"`);
        }
    }
    ...
    async similaritySearch(collectionId: string, data: SimilaritySearchQueryDTO) {
        try {
            ...
            this.handleError(error, `Failed to perform similarity search in collection "${collectionId}"`);
        }
    }
